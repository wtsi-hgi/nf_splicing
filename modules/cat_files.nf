process cat_filter_barcodes {
    ...
    output:
    tuple val(sample_id), path("${sample_id}.filter.barcodes.txt"), emit: ch_filter_barcodes
    ...
    cat header.txt se.txt pe.txt > ${sample_id}.filter.barcodes.txt
...
process cat_map_barcodes {
    ...
    output:
    tuple val(sample_id), path("${sample_id}.map.barcodes.txt"), emit: ch_map_barcodes
    ...
    cat header.txt se.txt pe.txt > ${sample_id}.map.barcodes.txt
    rm header.txt se.txt pe.txt
    """
}
